...
using TravelAway.Services.Controllers;
...
namespace TestApplication
{
    ...
    public class TestBusinessLayer
    {
        ...
        {
            int status;
            status = _adminBL.AddCustomer(custObj);
            if (status == 1)
            {
                TearDown();
                _testCount += 1;
        ...
        {
            bool status;
            status = _adminBL.AddAccomodation(accObj);
            if (status)
            {
                TearDownAcc();
                _testCount += 1;
        ...
        {
            int bookingId;
            bookingId = _adminBL.AddBookPackage(bkPkg);
            if (bookingId > 0)
            {
                TearDownBkPkg();
                _testCount += 1;
        ...
        {
            SetUp();
            int status = _adminBL.ValidateLoginCustomer(custObj.EmailId, custObj.UserPassword);
            if (status == 1)
            {
                _testCount += 1;
            }
        ...
        {
            SetUp();
            int status = _adminBL.ValidateLoginCustomer("invalid", custObj.UserPassword);
            if (status == 0)
            {
                _testCount += 1;
            }
        ...
        {
            SetUp();
            int status = _adminBL.ValidateLoginCustomer(custObj.EmailId, "invalid");
            if (status == 0)
            {
                _testCount += 1;
            }
